  ...
  module Importers
    class TTXML < Importer
      ...
      def self.specify_options(c)
        c.option 'source', '--source FILE', 'WordPress export XML file (default: "textcube-backup.xml")'
        c.option 'no_fetch_attachments', '--no-fetch-attachments', 'Do not fetch the attachments'
      ...
      # TODO: change implementation to fit with TTXML
      def self.extract_attachments(title, post_hpricot, assets_folder)
      ...
      def self.process(options)
        source        = options.fetch('source', "textcube-backup.xml")
        fetch         = !options.fetch('no_fetch_attachments', false)
        ...
        (doc/:channel/:item).each do |item|
          ...
          begin
            ...
            # TODO: implement TTML/Markdown/TinyMCE format conversion
